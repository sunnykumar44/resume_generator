...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  h2 { font-size: 13px; margin: 14px 0 6px; border-bottom: 1.5px solid #2b6cb0; color: #1a365d; text-transform: uppercase; letter-spacing: 1px; }
...
<body>
  <div style="width: 100%; text-align: center !important; margin-bottom: 15px;">
    <h1 style="font-size: 30px; font-weight: 800; color: #1a365d; text-transform: uppercase; margin-bottom: 0px;">${userProfile.name}</h1>
  ...
  <h2>Professional Summary</h2><div class="section"><p>[AI: Tailored summary]</p></div>
  <h2>Technical Skills</h2><div class="section">[AI: Comprehensive categories]</div>
  <h2>Education</h2>
  <div class="section"><div class="split-row"><strong>${userProfile.education.degree}</strong><span style="font-weight:bold;">${userProfile.education.year}</span></div><p>${userProfile.education.institution}</p></div>
  ...
  <h2>Selected Projects</h2><div class="section">[AI: 2 projects. Max 3 bullets each.]</div>
  <h2>Certifications & Expertise</h2><div class="section">[AI: List user certs + 1 suggested cert.]</div>
  <h2>Key Achievements</h2><div class="section"><ul>[AI: EXACTLY 3 points.]</ul></div>
  <div style="display:flex; justify-content:space-between; margin-top:20px; border-top:1px solid #cbd5e1; padding-top:10px;">
    ...
    // 2. Call the Generate Content method with the new SDK structure
    const response = await ai.models.generateContent({
      model: 'models/gemini-2.5-pro', // Using the Pro model ID from your list
      contents: [{ role: 'user', parts: [{ text: prompt }] }]
    });

    let html = response.text; // Use .text property directly
